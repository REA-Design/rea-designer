...
if(portfolioGrid){
            ...
            if(dataType === "story"){
                ...
                card.addEventListener("click", function(){
                    openLightbox(img.src, img.alt, "story", [
                        { label: "سوشيال ميديا", action: function(){ goToSocialTab("reels"); } },
                        { label: "ستوري", action: null }
                    ], story.title, "stories:" + story.id, "هل لديك استفسار؟");
                });
                return;
            }
            ...
            if(dataType === "post-image"){
                ...
                card.addEventListener("click", function(){
                    openLightbox(img.src, img.alt, null, [
                        { label: "سوشيال ميديا", action: function(){ goToSocialTab("reels"); } },
                        { label: "بوستات", action: function(){ goToSocialTab("posts"); } },
                        { label: group.title, action: null }
                    ], group.title, "posts:" + group.id, "هل لديك استفسار؟");
                });
                return;
            }
            ...
            if(cat === "business-cards" || cat === "branding"){
                card.addEventListener("click", function(){
                    ...
                    if(img){
                        ...
                        ], titleText, cat + ":" + card.dataset.id, "ابدأ مشروعك الآن");
                    }
                });
            }
        });
    ...
    function setBreadcrumb(extraParts){
        ...
        const parts = [
            { label: "الرئيسية", action: function(){ window.location.href = "index.html#home"; } },
            { label: "معرض الأعمال", action: function(){
                forceCloseAllOverlays();
                window.scrollTo({ top: 0, behavior: "smooth" });
            } }
        ].concat(extraParts || []);
    ...
    function openLightbox(src, alt, mode, breadcrumbParts, shareTitle, shareParam, ctaLabel){
        ...
        if(lightboxCta){ lightboxCta.textContent = ctaLabel || "هل لديك استفسار؟"; }
    ...
    (function openFromURL(){
        ...
        setTimeout(function(){
            ...
            if(type === "business-cards" || type === "branding"){
                ...
                ], project.title, type + ":" + id, "ابدأ مشروعك الآن");
                return;
            }
            if(type === "posts"){
                const group = socialData.posts.find(function(p){ return p.id === id; });
