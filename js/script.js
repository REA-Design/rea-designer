...
if(portfolioGrid){
                ...
                card.addEventListener("click", function(){
                    ...
                    if(img){
                        openLightbox(img.src, img.alt, null, [
                            { label: categoryLabels2[cat], action: null },
                            { label: (title ? title.textContent : img.alt), action: null }
                        ]);
                    }
                });
            }
    ...
    function setBreadcrumb(extraParts){
        ...
        const parts = [
            ...
            { label: "معرض الأعمال", action: function(){ popOverlay(); } }
        ].concat(extraParts || []);
    ...
    function openLightbox(src, alt, mode, breadcrumbParts){
        ...
        portfolioGrid.querySelectorAll(".portfolio-item").forEach(function(card){
            ...
            card.addEventListener("click", function(){
                openLightbox(img.src, img.alt, "story", [
                    { label: "سوشيال ميديا", action: null },
    ...
    function renderPostGalleryFolders(activeId){
        ...
        postGalleryFoldersWrap.querySelectorAll(".folder-pill").forEach(function(btn){
            btn.addEventListener("click", function(){
                ...
                if(group){ switchPostGalleryGroup(group); }
            });
        });
    ...
    /* التنقل من مجموعة لمجموعة تانية (بانرات أسماء المجلدات) بيحدّث
       المحتوى فقط بدون ما يضيف خطوة جديدة لذاكرة المتصفح */
    function switchPostGalleryGroup(group){
    ...
    function openProjectDetail(category, id){
        ...
        setBreadcrumb([
            { label: categoryLabels2[category] || category, action: null },
        ...
        if(shareBtn){
            shareBtn.onclick = function(){
                const shareData = {
                    title: "REA.DESIGNER — " + project.title,
                    text: project.title + " | " + (project.subtitle || "REA.DESIGNER"),
                ...
                if(navigator.share){
                    ...
                    navigator.clipboard.writeText(shareData.url).then(function(){
                        ...
                        setTimeout(function(){
                            shareBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="18" cy="5" r="2.6" stroke="currentColor" stroke-width="1.7"/><circle cx="6" cy="12" r="2.6" stroke="currentColor" stroke-width="1.7"/><circle cx="18" cy="19" r="2.6" stroke="currentColor" stroke-width="1.7"/><path d="M8.3 10.7 15.7 6.6M8.3 13.3 15.7 17.4" stroke="currentColor" stroke-width="1.7"/></svg> مشاركة';
                        }, 1800);
                    });
                }
            };
...
if(langToggle){
    langToggle.addEventListener("click", function(){
        ...
        if(!note){
            ...
            note.textContent = "النسخة الإنجليزية قريباً 🌍";
            document.body.appendChild(note);
        }
        note.classList.add("active");
        clearTimeout(langToggle._toastTimer);
        langToggle._toastTimer = setTimeout(function(){
