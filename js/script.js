...
if(portfolioGrid){
    ...
    function renderMixedAll(){
        ...
        allCategoriesOrder.forEach(function(cat){
            portfolioData[cat].forEach(function(item){
                allItems.push({ item: item, cat: cat });
            });
        });

        const visible = allItems.slice(0, allRenderedCount);
        portfolioGrid.innerHTML = visible.map(function(x){
            return cardTemplate(x.item, "default", x.cat);
        }).join("");
    ...
    /* بحث خفيف: بيفلتر الكروت الظاهرة حالياً بالشبكة حسب الاسم/الوصف،
       بدون ما يفتح صفحة نتائج منفصلة أو يلمس تقسيمة البانرات */
